...
import type { Member, Port, PackageType } from "@/types";
import { PACKAGES } from "@/types";

type Props = { members: Member[]; ports: Port[] };

export default function AdminClient({ members, ports }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<"members" | "ports">("members");
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <main className="mx-auto max-w-5xl px-6 py-8">
        <div className="mb-6 flex gap-2">
          {(["members", "ports"] as const).map((tab) => (
            <button
              key={tab}
            ...
            >
              {tab === "members" ? `👥 สมาชิก (${memberList.length})` : `🔌 พอร์ต (${ports.filter((p) => p.status === "active").length})`}
            </button>
          ))}
        </div>
