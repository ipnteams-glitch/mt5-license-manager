...
function paymentFromRow(row: string[]): Payment {
  return {
    ...
    qr_payload: row[8] || "",
    agent_code: row[9] || undefined,
    agent_commission: row[10] ? parseFloat(row[10]) : undefined,
  };
}
...
export async function findPortByAccount(mt5Account: string, broker?: string): Promise<Port | null> {
  ...
  if (broker) {
    ...
    return all.find((p) =>
      p.mt5_account === mt5Account &&
      p.status === "active" &&
      (p.mt5_broker.toLowerCase().includes(brokerLower) ||
       brokerLower.includes(p.mt5_broker.toLowerCase()))
    ) || null;
  }
...
export async function deletePortfolioAccount(id: string, email: string): Promise<void> {
  ...
  const ps = await getPortSystems(mt5Account);
  // allow delete even if not in port_systems
...
// ── Agent Withdrawals (ถอนค่าคอม) ──
const WITHDRAWALS_SHEET = "agent_withdrawals";
function wdFromRow(r: string[]): AgentWithdrawal { return { id: r[0]||"", agent_code: r[1]||"", amount: parseFloat(r[2])||0, status: (r[3] as "pending"|"paid")||"pending", bank_name: r[4]||"", bank_account: r[5]||"", created_at: r[6]||"", paid_at: r[7]||"" }; }
function wdToRow(w: AgentWithdrawal): string[] { return [w.id,w.agent_code,String(w.amount),w.status,w.bank_name,w.bank_account,w.created_at,w.paid_at||""]; }
async function initWdsSheet() { const sheets=await getSheets();const sid=sheetId();try{await sheets.spreadsheets.values.get({spreadsheetId:sid,range:WITHDRAWALS_SHEET+"!A1"});}catch{const sp=await sheets.spreadsheets.get({spreadsheetId:sid});await sheets.spreadsheets.batchUpdate({spreadsheetId:sid,requestBody:{requests:[{addSheet:{properties:{title:WITHDRAWALS_SHEET}}}]}});await sheets.spreadsheets.values.append({spreadsheetId:sid,range:WITHDRAWALS_SHEET+"!A:H",valueInputOption:"RAW",requestBody:{values:[["id","agent_code","amount","status","bank_name","bank_account","created_at","paid_at"]]}});} }
export async function createWithdrawal(agent: Agent, amount: number): Promise<AgentWithdrawal> { await initWdsSheet();const pendingWds=await getWithdrawals(agent.agent_code);const pendingSum=pendingWds.filter(w=>w.status==="pending").reduce((s,w)=>s+w.amount,0);const avail=agent.commission_earned-agent.commission_paid-pendingSum;if(amount<=0||amount>avail) throw new Error("ยอดถอนไม่ถูกต้อง (คงเหลือ "+avail.toFixed(2)+")");const w: AgentWithdrawal={id:uuidv4(),agent_code:agent.agent_code,amount,status:"pending",bank_name:agent.bank_name,bank_account:agent.bank_account,created_at:new Date().toISOString()};const sheets=await getSheets();await sheets.spreadsheets.values.append({spreadsheetId:sheetId(),range:WITHDRAWALS_SHEET+"!A:H",valueInputOption:"RAW",requestBody:{values:[wdToRow(w)]}});invalidateCache("withdrawals");return w; }
export async function getWithdrawals(agent_code: string): Promise<AgentWithdrawal[]> { await initWdsSheet();const sheets=await getSheets();const res=await sheets.spreadsheets.values.get({spreadsheetId:sheetId(),range:WITHDRAWALS_SHEET+"!A:H"});const rows=res.data.values;if(!rows||rows.length<=1) return[];return rows.slice(1).filter(r=>r[1]===agent_code).map(wdFromRow); }
export async function getAllWithdrawals(): Promise<AgentWithdrawal[]> { await initWdsSheet();const sheets=await getSheets();const res=await sheets.spreadsheets.values.get({spreadsheetId:sheetId(),range:WITHDRAWALS_SHEET+"!A:H"});const rows=res.data.values;if(!rows||rows.length<=1) return[];return rows.slice(1).filter(r=>r[0]).map(wdFromRow); }
export async function markWithdrawalPaid(wdId: string): Promise<void> { const all=await getAllWithdrawals();const idx=all.findIndex(w=>w.id===wdId);if(idx===-1) throw new Error("Not found");const sheets=await getSheets();const sid=sheetId();await sheets.spreadsheets.values.update({spreadsheetId:sid,range:WITHDRAWALS_SHEET+"!D"+(idx+2)+":H"+(idx+2),valueInputOption:"RAW",requestBody:{values:[["paid",all[idx].bank_name,all[idx].bank_account,all[idx].created_at,new Date().toISOString()]]}});const agents=await getAllAgents();const ai=agents.findIndex(a=>a.agent_code===all[idx].agent_code);if(ai>=0){agents[ai].commission_paid+=all[idx].amount;const s2=await getSheets();await s2.spreadsheets.values.update({spreadsheetId:sid,range:"agents!I"+(ai+2),valueInputOption:"RAW",requestBody:{values:[[String(agents[ai].commission_paid)]]}});invalidateCache("agents");}invalidateCache("withdrawals"); }
